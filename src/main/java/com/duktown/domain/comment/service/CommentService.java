    ...
    public void createComment(Long userId, CommentDto.CreateRequest request){
        ...
        if(request.getDeliveryId() != null){
            ...
            if (!commentRepository.existsByUserIdAndDeliveryId(userId, delivery.getId())) {
                anonymousNumber = commentRepository.countUniqueUsersByDeliveryId(delivery.getId()) + 1;
            } else {
                //댓글 단 내역이 있다면 유저 아이디와 게시글 아이디로 댓글 한 개만 찾아 해당 댓글의 익명 숫자를 현재 댓글 익명 숫자로 부여
                anonymousNumber = commentRepository.findFirstByUserIdAndDeliveryId(userId, delivery.getId())
                        .getAnonymousNumber();
            }
            ...
            if (!commentRepository.existsByUserIdAndPostId(userId, post.getId())) {
                anonymousNumber = commentRepository.countUniqueUsersByPostId(post.getId()) + 1;
            } else {
                anonymousNumber = commentRepository.findFirstByUserIdAndPostId(userId, post.getId())
                        .getAnonymousNumber();
            }
        } else {
        ...
        Comment comment = request.toEntity(user, delivery, post, parentComment, anonymousNumber);
